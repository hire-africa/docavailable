<?php

// Test video call API
$apiUrl = 'https://docavailable-3vbdv.ondigitalocean.app/api';

$token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..lythdGH1_mNEj8Wx-_4gjz5qJVMZXj2G7J1fnkC2RjY';

echo "=== TESTING VIDEO CALL API ===\n";

// Test call availability check for video
$testData = [
    'call_type' => 'video'
];

$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $apiUrl . '/call-sessions/check-availability');
curl_setopt($ch, CURLOPT_POST, true);
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($testData));
curl_setopt($ch, CURLOPT_HTTPHEADER, [
    'Content-Type: application/json',
    'Accept: application/json',
    'Authorization: Bearer ' . $token
]);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);

$response = curl_exec($ch);
$httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
curl_close($ch);

echo "Video Call Availability Check:\n";
echo "HTTP Code: $httpCode\n";
$data = json_decode($response, true);
echo "Response: " . json_encode($data, JSON_PRETTY_PRINT) . "\n\n";

// Test video call start
echo "=== TESTING VIDEO CALL START ===\n";
$testData = [
    'call_type' => 'video',
    'appointment_id' => 'test_video_session_' . time(),
    'doctor_id' => 1,
    'reason' => 'Test video call session'
];

$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $apiUrl . '/call-sessions/start');
curl_setopt($ch, CURLOPT_POST, true);
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($testData));
curl_setopt($ch, CURLOPT_HTTPHEADER, [
    'Content-Type: application/json',
    'Accept: application/json',
    'Authorization: Bearer ' . $token
]);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);

$response = curl_exec($ch);
$httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
curl_close($ch);

echo "Video Call Start Test:\n";
echo "HTTP Code: $httpCode\n";
$data = json_decode($response, true);
echo "Response: " . json_encode($data, JSON_PRETTY_PRINT) . "\n";

if ($data && $data['success'] === true) {
    echo "✅ SUCCESS! Video call session created successfully!\n";
} else {
    echo "❌ Video call session creation failed\n";
}
