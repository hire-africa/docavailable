...
try {
    ...
    echo "📦 QUEUE JOBS FOR SESSION 105:\n";
    ...
    echo "   Found " . count($sessionJobs) . " jobs for session 105\n";
    ...
    if (count($sessionJobs) > 0) {
        ...
        echo "   No pending jobs found for session 105\n";
    }
    
    // Check if the session was properly activated
    echo "\n🔧 ACTIVATION ANALYSIS:\n";
    if ($session->activated_at) {
        echo "   ✅ Session was activated at: {$session->activated_at}\n";
        echo "   Total allowed minutes: {$session->getTotalAllowedMinutes()}\n";
        echo "   Should auto-end due to insufficient sessions: " . ($session->shouldAutoEndDueToInsufficientSessions() ? 'YES' : 'NO') . "\n";
    } else {
        echo "   ❌ Session was never activated!\n";
        echo "   This means the doctor never sent the first message.\n";
    }
    ...
    if ($session->auto_deductions_processed == 0 && $session->sessions_used == 0) {
        ...
        echo "   3. Session was never properly activated\n";
    ...
    echo "\n💡 RECOMMENDATIONS:\n";
    echo "   1. Check if queue jobs were created when session 105 was activated\n";
    echo "   2. Verify that the queue processing middleware is working\n";
    echo "   3. Check if the session was properly activated by the doctor\n";
