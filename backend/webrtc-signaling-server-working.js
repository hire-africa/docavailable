...
function safeSend(ws, message) {
  try {
    if (ws && ws.readyState === WebSocket.OPEN) {
      // Send message without compression
...
const handleConnection = (ws, req, connectionType) => {
  ...
  // Send connection confirmation with proper error handling
  safeSend(ws, {
    ...
    try {
      ...
      switch (data.type) {
        case 'offer':
          ...
          sendGlobalIncomingCallNotification(appointmentId, data);
  ...
  // Handle WebSocket ping/pong to keep connection alive
  ws.on('ping', (data) => {
    console.log('🏓 Ping received, sending pong');
    try {
      if (ws.readyState === WebSocket.OPEN) {
        // Send pong with the same data as ping (WebSocket spec requirement)
        // Use false to disable compression
        ws.pong(data, false);
      }
    } catch (error) {
...
// Add heartbeat to keep connections alive (temporarily disabled for debugging)
// setInterval(() => {
//   connections.forEach((appointmentConnections, appointmentId) => {
//     appointmentConnections.forEach(connection => {
//       if (connection.readyState === WebSocket.OPEN) {
//         try {
//           // Send ping without compression
//           connection.ping('heartbeat', false);
//         } catch (error) {
...
// Helper function to send global incoming call notification
    ...
    globalConnections.forEach((connection, index) => {
      try {
        if (connection.ws && connection.ws.readyState === WebSocket.OPEN) {
          connection.ws.send(JSON.stringify(notificationData));
  ...
  appointmentConnections.forEach((connection, index) => {
    if (connection !== senderWs && connection.readyState === WebSocket.OPEN) {
      try {
        connection.send(JSON.stringify(data));
        sentCount++;
        console.log(`📤 [Broadcast] Sent to connection ${index + 1} (${connection.readyState})`);
      } catch (error) {
